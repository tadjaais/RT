

# 👩🏽‍💻 Aïssata Raoufa Touré  

**Data Analyst - DataScientist | Orange Côte d’Ivoire | Passionnée de Data & Stratégie**

📍 Abidjan, Côte d’Ivoire  
🌐 [LinkedIn](https://www.linkedin.com/in/raoufa-tour%C3%A9) | 💻 [GitHub](https://github.com/tadjaais) | ✉️ toureraoufa@gmail.com  

---

## 🌟 À propos de moi  

Je suis **Data Analyst** dans le secteur des **télécommunications**, spécialisée dans l’analyse de la performance commerciale et la création de **tableaux de bord dynamiques** pour le pilotage stratégique.  
Mon objectif : **transformer la donnée en décisions éclairées** et aider les équipes à atteindre l’excellence opérationnelle.

💡 Curieuse, rigoureuse et orientée résultats, j’aime concevoir des analyses claires, utiles et impactantes.  

---

## 🛠️ Compétences Techniques  

| Domaine | Outils & Technologies |
|----------|----------------------|
| **Langages** | ![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white) ![R](https://img.shields.io/badge/R-276DC3?style=flat&logo=r&logoColor=white) ![SQL](https://img.shields.io/badge/SQL-336791?style=flat&logo=postgresql&logoColor=white) |
| **Visualisation** | ![Power BI](https://img.shields.io/badge/PowerBI-F2C811?style=flat&logo=powerbi&logoColor=black) ![Tableau](https://img.shields.io/badge/Tableau-E97627?style=flat&logo=tableau&logoColor=white) ![Shiny](https://img.shields.io/badge/Shiny-4479A1?style=flat&logo=rstudio&logoColor=white) |
| **Statistiques & ML** | ACP, K-means, Régression linéaire & logistique |
| **Data Management** | Excel Avancé, Nettoyage & Transformation, DAX |
| **Soft Skills** | Esprit analytique, Adaptabilité, Communication data-driven |

---

## 📊 Projets phares  

### 🚀 [Prévision des ventes Fibres – Étude de comparaison des modeleles de série temporelles VS Machine learning](https://github.com/tadjaais/Projet-machine-learning.git)
> **Objectif :** Projet de prévision des ventes de services Fibre, comparant des modèles traditionnels de séries temporelles (ARIMA, Prophet) et des modèles de machine learning (Random Forest, XGBoost). Le projet inclut la préparation des données, l’entraînement des modèles, l’évaluation de leurs performances et l’identification du modèle le plus précis pour anticiper les ventes futures.  
> **Techniques :** ARIMA,SARIMA,Random Forest, XGBoost,métriques RMSE (Root Mean Squared Error) et MAPE (Mean Absolute Percentage Error),Visualisation et comparaison des prévisions vs valeurs réelles  
> **Résultat :** les modèles de machine laerning ont surpassé les modèles de séries temporelles en précision,la regression linéaire offrant des prévisions fiables des ventes de Fibre.  

---

### 🚗 [Analyse des sinistres d’assurance auto](https://github.com/ton-lien/assurance-auto)
> **Objectif :** Modéliser la fréquence et le coût des sinistres.  
> **Techniques :** Régressions, ACP, visualisations ggplot2.  
> **Résultat :** Prédiction des risques clients et optimisation tarifaire.  

---

### 📈 [Dashboard Power BI – Suivi des ventes FTTH & Broadband](https://github.com/ton-lien/dashboard-ftth)
> **Objectif :** Piloter les ventes B2B et DRDI avec des indicateurs dynamiques.  
> **Outils :** Power BI, Excel, DAX.  
> **Résultat :** Vision claire des performances et de l’atteinte des objectifs commerciaux.  

---

### 💡 [Application Shiny – Analyse Multidimensionnelle Interactive](https://github.com/ton-lien/shiny-acp)
> **Objectif :** Offrir une visualisation interactive des résultats d’ACP.  
> **Techniques :** R Shiny, Factoshiny, ggplot2.  
> **Résultat :** Application web intuitive pour explorer les individus et variables.  

---

## 📚 Formation Académiques 

🎓 **Master en Data Science** – Institut Supérieur de Statistiques , d'Econométrie et de DataScience - Côte d’Ivoire  

🎓 **Licence en Statistiques et Économie**  – Université Polytechnique de Bingerville - Côte d’Ivoire

🎓 **Baccalauréat scientifique - série C**  – Lycée Scientifique de Yamoussoukro - Côte d’Ivoire

---

## 🚀 Ce qui me motive  

> “La donnée n’a de valeur que lorsqu’elle éclaire une décision.”  

Je crois au pouvoir de la **data** pour révéler des opportunités et soutenir la **prise de décision stratégique**.  
Je cherche toujours à apprendre, à innover et à contribuer à des projets à fort impact.  

---

## 📫 Me contacter  

📧 **Email :** toureraoufa@gmail.com  
🌐 **LinkedIn :** [Aïssata Raoufa Touré](https://www.linkedin.com/in/raoufa-tour%C3%A9) 
💻 **GitHub :** [@RaoufaToure](https://github.com/tadjaais) 

---

⭐️ _“Apprendre, analyser, agir : la donnée est mon langage.”_  
